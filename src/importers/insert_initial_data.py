...
from src.utils.import_helpers import ImportResult, create_import_result


def main() -> ImportResult:
    ...
    try:
        ...
        with get_db_context() as db:
            ...
            all_weights = (
                db.query(BodyMetric)
                .filter(
                    BodyMetric.user_id == user.id, BodyMetric.metric_type == "weight"
                )
        ...
        return create_import_result(
            script_name="insert_initial_data",
            success=True,
            inserted_count=0,  # This script doesn't insert in bulk
            skipped_count=0,
